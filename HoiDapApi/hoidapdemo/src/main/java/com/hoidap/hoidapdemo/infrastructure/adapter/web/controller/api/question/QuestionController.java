...
public class QuestionController {
    ...
    public ResponseEntity<Resource> downloadFile(@PathVariable Long id) {
        ...
        return ResponseEntity.ok()
                .contentType(
                        MediaType.parseMediaType(Objects.requireNonNull(q.getFileType(), "File type không được null")))
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + q.getFileName() + "\"")
                .body(new ByteArrayResource(Objects.requireNonNull(q.getFileData(), "File data không được null")));
    }
    ...
    public ResponseEntity<Resource> downloadAnswerVersionFile(@PathVariable Long versionId) {
        ...
        return ResponseEntity.ok()
                .contentType(MediaType
                        .parseMediaType(Objects.requireNonNull(version.getFileType(), "File type không được null")))
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + version.getFileName() + "\"")
                .body(new ByteArrayResource(
                        Objects.requireNonNull(version.getFileData(), "File data không được null")));
    }
